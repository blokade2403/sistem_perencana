...
class PdfControllerRkbuPersediaan extends Controller
{
    public function downloadPDFPersediaan(Request $request)
    {
        ...
        $requiredSpace = 30; // Tentukan tinggi yang diperlukan untuk elemen (misalnya tabel)
        $this->checkPageBreak($pdf, $requiredSpace);

        $htmlBody = view('frontend.prints.print_rkbu_persediaan', compact('rkbus', 'nama_tahun_anggaran', 'angka_tahun', 'total_anggaran_terbilang', 'sumberDana', 'jabatan_kabag', 'nama_rumah_sakit', 'nama_pejabat', 'nip_pejabat_kabag', 'nip_direktur', 'nama_direktur', 'pdf', 'sub_kategori_rkbus', 'status_validasi_rka', 'total_anggaran', 'categories', 'namaProgram', 'namaAktivitas', 'namaKegiatan', 'namaSubKegiatan', 'kodeProgram', 'kodeKegiatan', 'kodeSubKegiatan'))->render();
    ...
    public function checkPageBreak($pdf, $requiredSpace)
    {
        // Dapatkan posisi Y saat ini di halaman
        $currentY = $pdf->GetY();
        // Dapatkan tinggi halaman yang tersisa
        ...
        // Jika ruang yang tersedia lebih kecil dari ruang yang diperlukan, tambahkan halaman baru
