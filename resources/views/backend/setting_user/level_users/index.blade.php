...
<div class="content-wrapper">
    <!-- Content -->
                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('level_users.create')}}" class="btn btn-outline-secondary btn-primary">
        ...
        @else
                                <tr>
                                    @php $no = 1; @endphp
                                    @foreach ($level_users as $key)
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $key->id_level }}</td>
                                    <td>{{ $key->nama_level_user }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('level_users.edit', $key->id_level) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('level_users.destroy', $key->id_level) }}" method="POST" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
