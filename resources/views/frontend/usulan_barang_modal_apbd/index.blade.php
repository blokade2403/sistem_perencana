...
@section('container')
    <div class="card-body">
                                    ...
                                    <div class="user-profile-info">
                                        ...
                                        <ul class="list-inline mb-0 d-flex align-items-center flex-wrap justify-content-sm-start justify-content-center gap-2">
                                            ...
                                            <div class="demo-inline-spacing">
                                                <button class="btn rounded-pill btn-label-info waves-effect" data-bs-toggle="modal" data-bs-target="#referAndEarnModal">
                            ...
                            <table id="example" class="table table-striped" style="width:100%">
                                ...
                                <tbody>
                                    @php
                                            ...
                                            <tr>
                                            <td>{{$no++}}</td>
                                            <td class="fs-14 text-wrap" ><small>No. ID <br />{{$key->sub_kategori_rkbu->kode_sub_kategori_rkbu}}. {{$key->sub_kategori_rkbu->nama_sub_kategori_rkbu}}</small></td>
                                            <td class="fs-14"><strong><small><span class="fs-14 text-primary">{{$key->no_usulan_barang}}</span></small></strong><br /><span><small>Nama Pengusul : {{$key->nama_pengusul_barang}}</small></span></td>
                                            <td>
                                            ...
                                            @include('frontend.partials.modal_komponen_usulan_barang_modal')
                                            </td>
                                            <td><a href="{{ route('usulan_barang_modals.keranjang', $key->id_usulan_barang) }}" class="btn btn-sm btn-info">
                                                    <span class="tf-icons mdi mdi-cart-arrow-down me-1"></span>Lihat
                                                </a>
                                            </td>
                                            <td>
                                                @if ($key->status_usulan_barang == 'Pending')
                                                <span class="badge bg-label-danger">Pending</span>
                                                @else
                                                <span class="badge bg-label-info">Proses Pesanan Barang</span>
                                                @endif
                                                </td>
                                                <td>
                                                    @if (is_null($key->id_usulan_barang_detail))
                                                    <form action="{{ route('usulan_barang_modals.delete', $key->id_usulan_barang) }}" method="POST" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
        ...
        @include('frontend.partials.modal_usulan_barang_sub_kategori_modal')
