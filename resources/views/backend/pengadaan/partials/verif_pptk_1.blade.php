<form class="needs-validation" action="{{ route('master_spj.updateVerifPPTK', $master_spj->id_master_spj) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="card mb-4">
        @if ($master_spj->status_verifikasi_pptk == 'Selesai')
            <h5 class="card-header bg-label-success mb-4">Verifikasi PPTK</h5>
        @else
            <h5 class="card-header bg-label-info mb-4">Verifikasi PPTK</h5>
        @endif
        <div class="card-body">
                        ...
                        <div id="creditCardForm" class="row g-4">
                            <div class="col-12">
                                <span class="switch-label bg-label-danger rounded-1">
                                    <span class="fa-xs">Tanggal Verif PPTK: {{ $master_spj->tgl_verif_pptk ?? 'belum di verifikasi'}}</span>
                                </span>
                            </div>
                            <div class="col-12">
                                <div class="form-floating form-floating-outline">
                                    <select name="status_verifikasi_pptk" id="satuan_1" class="select2 form-select form-select-lg" data-allow-clear="true">
                                        <option value="{{ $master_spj->status_verifikasi_pptk }}">{{ $master_spj->status_verifikasi_pptk }}</option>
                            ...
                            <div class="col-12">
                                <div class="form-floating form-floating-outline">
                                    <textarea class="form-control h-px-75" name="ket_verif_pptk" aria-label="With textarea">{{$master_spj->ket_verif_pptk ?? '-'}}</textarea>
