...
class ValidasiUsulanBarangModalController extends Controller
{

    public function index(Request $request, $id_sub_kategori_rkbu = null)
    {
        ...
        $usulan_barangs = UsulanBarang::join('sub_kategori_rkbus', 'sub_kategori_rkbus.id_sub_kategori_rkbu', '=', 'usulan_barangs.id_sub_kategori_rkbu')
            ...
            ->whereIn('jenis_kategori_rkbus.id_jenis_kategori_rkbu', ['9cf70e1d-18e7-40fe-bdd3-b7dabf61877d', '9cf70e31-9b9e-4dea-8b39-5459f23f3f51']) // Kategori modal
        ...
        $sub_kategori_usulan_belanja = UsulanBarang::join('sub_kategori_rkbus', 'sub_kategori_rkbus.id_sub_kategori_rkbu', '=', 'usulan_barangs.id_sub_kategori_rkbu')
            ->join('kategori_rkbus', 'sub_kategori_rkbus.id_kategori_rkbu', '=', 'kategori_rkbus.id_kategori_rkbu')
            ->join('jenis_kategori_rkbus', 'kategori_rkbus.id_jenis_kategori_rkbu', '=', 'jenis_kategori_rkbus.id_jenis_kategori_rkbu')
            ->whereIn('jenis_kategori_rkbus.id_jenis_kategori_rkbu', ['9cf70e1d-18e7-40fe-bdd3-b7dabf61877d', '9cf70e31-9b9e-4dea-8b39-5459f23f3f51']) // Kategori modal
            ->where('status_usulan_barang', 'Selesai')
        ...
        $query = UsulanBarang::join('sub_kategori_rkbus', 'sub_kategori_rkbus.id_sub_kategori_rkbu', '=', 'usulan_barangs.id_sub_kategori_rkbu')
            ->join('kategori_rkbus', 'sub_kategori_rkbus.id_kategori_rkbu', '=', 'kategori_rkbus.id_kategori_rkbu')
            ->join('jenis_kategori_rkbus', 'kategori_rkbus.id_jenis_kategori_rkbu', '=', 'jenis_kategori_rkbus.id_jenis_kategori_rkbu')
            ->where('tahun_anggaran', $tahunAnggaran)
            ->where('status_usulan_barang', 'Selesai')
        ...
        // Cek apakah ada filter sub kategori yang dipilih
        if ($request->filled('sub_kategori_rkbu')) {
            $query->where('usulan_barangs.id_sub_kategori_rkbu', $request->sub_kategori_rkbu);
        }

        if ($request->filled('status_permintaan_barang')) {
            $query->where('usulan_barangs.status_permintaan_barang', $request->status_permintaan_barang);
        }
        ...
        return view('frontend.validasi_usulan_admin.validasi_usulan_barang_modal.index', compact('usulan_barangs', 'sub_kategori_usulan_belanja', 'get_barangs', 'id_usulan', 'sub_kategori_rkbus'));
    }
    ...
    public function updateValidasi(Request $request, $no_usulan_barang)
    {
        ...
        $request->validate([
            'status_validasi_perencana' => 'required',
            'keterangan_perencana' => 'nullable|string',
        ]);
        ...
        // Update data
        $usulanBarang->status_permintaan_barang = $request->input('status_validasi_perencana');
        $usulanBarang->tgl_validasi_perencana = now(); // Set tanggal validasi ke waktu saat ini
        $usulanBarang->keterangan_perencana = $request->input('keterangan_perencana');
        $usulanBarang->status_validasi_perencana = $request->input('status_validasi_perencana');
        $usulanBarang->nama_valid_perencana = auth()->user()->id_user; // Menyimpan id_user yang sedang login ke nama_valid_perencana

        // Pastikan data relasi ksp tidak null
        $nipKsp = $usulanBarang->validPerencana ? $usulanBarang->validPerencana->nip_user : 'NIP Tidak Ditemukan';
        $namaPengusul = $usulanBarang->validPerencana ? $usulanBarang->validPerencana->nama_lengkap : 'Pengusul Tidak Ditemukan';

        // Data untuk QR code
        $dataQR = "Telah divalidasi oleh: " . $namaPengusul .
            "\nNIP: " . $nipKsp .
            "\nNo Usulan: " . $usulanBarang->no_usulan_barang .
            "\nTanggal: " . $usulanBarang->tgl_validasi_perencana->format('d-m-Y');
        ...
        $qrcodePath = 'qrcodes/qrcode_admin' . $usulanBarang->no_usulan_barang . '.png';
        Storage::disk('public')->put($qrcodePath, $qrcodeImage);

        // Simpan path QR code ke kolom qrcode_pengusul
        $usulanBarang->qrcode_perencana = $qrcodePath;
        ...
        return redirect()->route('validasi_usulan_barang_modals.index')
    ...
    public function keranjang($id_usulan_barang)
    {
        ...
        return view('frontend.validasi_usulan_admin.validasi_usulan_barang_modal.keranjang', $data);
    }
    ...
    public function show($id_usulan_barang)
    {
        ...
        return view('frontend.validasi_usulan_admin.validasi_usulan_barang.invoice', $data, compact('sub_kategori_rkbus', 'usulan_barangs', 'id_usulan', 'invoice'));
    }
    ...
    public function doDelete($id_usulan_barang)
    {
        ...
        return redirect()->route('validasi_usulan_barang_modals.index')->with('success', 'Data usulan barang berhasil dihapus.');
    }
}
