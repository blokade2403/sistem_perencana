                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('admin_pendukungs.create')}}" class="btn btn-outline-secondary btn-primary">
        ...
        @else
                                <tr>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($admin_pendukungs as $key)
                                    <td>{{ $no++}}</td>
                                    <td>{{ $key->nama_pendukung_ppk }} </td>
                                    <td>{{ $key->id_user }} </td>
                                    <td>{{ $key->jabatan_pendukung_ppk }} </td>
                                    <td>{{ $key->nrk }}</td>
                                    <td>{{ $key->id_pejabat_pengadaan }}</td>
                                    <td>{{ $key->id_pptk }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('admin_pendukungs.edit', $key->id_admin_pendukung_ppk) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('admin_pendukungs.destroy', $key->id_admin_pendukung_ppk) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
