...
class KertasKerjaApbd extends Controller
{
    public function index(Request $request)
    {
        ...
        $categories = JenisBelanja::whereHas('jenis_kategori_rkbus.obyek_belanjas.kategori_rkbus.sub_kategori_rkbus.rkbus.rekening_belanjas.aktivitas.programs', function ($query) use ($tahunAnggaran) {
        ...
        $jabatan_direktur = Pejabat::join('jabatans', 'jabatans.id_jabatan', '=', 'pejabats.id_jabatan')
            ->select('pejabats.nama_pejabat')
            ->where('pejabats.id_jabatan', '9cdfc12b-061a-4336-a8fd-6c51103a4c72')
            ->first();

        $bendahara = PejabatPengadaan::select('nama_bendahara')->first();
        ...
        return view('backend.ringkasan.kertas_kerja.realisasi_rba_apbd', compact(
            'rkbus',
            'judul_header1',
            'nama',
            'jabatan_direktur',
