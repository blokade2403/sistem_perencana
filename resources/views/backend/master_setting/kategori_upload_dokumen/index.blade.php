                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('kategori_upload_dokumens.create')}}" class="btn btn-outline-secondary btn-primary">
                    ...
                    <div class="card">
                        ...
                        <table id="example" class="table table-bordered" style="min-width: 845px">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Nama Kategori Upload</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if($kategori_upload_dokumen->isEmpty())
                                    <div class="alert alert-danger text-center">
                                        Tidak ada data yang tersedia.
                                    </div>
                                @else
                                <tr>
                                    @foreach ($kategori_upload_dokumen as $key)
                                    <td>{{ $key->id_kategori_upload_dokumen }}</td>
                                    <td>{{ $key->nama_kategori_upload_dokumen }}</td>
                                    <td>
                                        <div class="btn-group" id="dropdown-icon-demo">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a href="{{ route('kategori_upload_dokumens.edit', $key->id_kategori_upload_dokumen) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                </li>
                                                <form action="{{ route('kategori_upload_dokumens.destroy', $key->id_kategori_upload_dokumen) }}" method="POST" class="delete-form">
                                                    @csrf
                                                    @method('DELETE')
