                                    ...
                                    <div class="d-flex align-items-md-end align-items-sm-start align-items-center justify-content-md-between justify-content-start mx-4 flex-md-row flex-column gap-4">
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <a href="{{route('kategori_rkbus.create')}}" class="btn rounded-pill btn-label-info waves-effect">
                        ...
                        <table id="example" class="table table-bordered" style="min-width: 845px">
                            <thead>
                                <tr>
                                    ...
                                    <th>Nama Kategori RKBU</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if($kategori_rkbus->isEmpty())
                                ...
                                <tr>
                                    @php $no = 1; @endphp
                                    @foreach ($kategori_rkbus as $ksp)
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $ksp->obyek_belanja->kode_obyek_belanja }}. {{ $ksp->obyek_belanja->nama_obyek_belanja }}</td>
                                    <td>{{ $ksp->jenis_kategori_rkbu->kode_jenis_kategori_rkbu }}. {{ $ksp->jenis_kategori_rkbu->nama_jenis_kategori_rkbu }}</td>
                                    <td>{{ $ksp->kode_kategori_rkbu }}. {{ $ksp->nama_kategori_rkbu }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('kategori_rkbus.edit', $ksp->id_kategori_rkbu) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('kategori_rkbus.destroy', $ksp->id_kategori_rkbu) }}" method="POST" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
