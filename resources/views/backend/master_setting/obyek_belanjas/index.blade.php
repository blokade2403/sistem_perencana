...
<div class="content-wrapper">
    <!-- Content -->
        ...
        <!-- Content -->
            <div class="card-body">
                @include('backend.partials.modal_upload_kegiatan')
                <!-- Header -->
                <div class="row">
                    <div class="col-12">
                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('obyek_belanjas.create')}}" class="btn btn-outline-secondary btn-primary">
                                                        <i class="mdi mdi-account-check-outline me-1">Add</i>
                                                    </a>
                                                    <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#referAndEarn">
                                                        <i class="tf-icons mdi mdi-upload-outline"></i>
                                                    </button>
                    ...
                    <div class="card">
                        ...
                        <table id="example" class="table table-bordered" style="min-width: 845px">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Jenis Kategori RKBU</th>
                                    <th>Kode Obyek Belanja</th>
                                    <th>Nama Obyek Belanja</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if($obyek_belanjas->isEmpty())
        ...
        @else
                                <tr>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($obyek_belanjas as $key)
                                    <td>{{ $no++}}</td>
                                    <td>{{ $key->jenis_kategori_rkbu->kode_jenis_kategori_rkbu }}. {{ $key->jenis_kategori_rkbu->nama_jenis_kategori_rkbu }}</td>
                                    <td>{{ $key->kode_obyek_belanja }}</td>
                                    <td>{{ $key->nama_obyek_belanja }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('obyek_belanjas.edit', $key->id_obyek_belanja) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('obyek_belanjas.destroy', $key->id_obyek_belanja) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
