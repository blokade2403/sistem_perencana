                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('kategori_rekenings.create')}}" class="btn btn-outline-secondary btn-primary">
                    ...
                    <div class="card">
                        ...
                        <table id="example" class="table table-bordered" style="min-width: 845px">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Kode Kategori Rekening</th>
                                    <th>Nama Kategori Rekening</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if($kategori_rekenings->isEmpty())
        ...
        @else
                                <tr>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($kategori_rekenings as $key)
                                    <td>{{ $no++}}</td>
                                    <td>{{ $key->kode_kategori_rekening }}</td>
                                    <td>{{ $key->nama_kategori_rekening }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('kategori_rekenings.edit', $key->id_kategori_rekening) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('kategori_rekenings.destroy', $key->id_kategori_rekening) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                            ...
                            <tfoot>
                                <th>No</th>
                                    <th>Kode kategori_rekening</th>
                                    <th>Nama kategori_rekening</th>
                                    <th>Action</th>
                            </tfoot>
                        </table>
                    </div>
