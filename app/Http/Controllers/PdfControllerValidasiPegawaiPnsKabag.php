...
class PdfControllerValidasiPegawaiPnsKabag extends Controller
{
    public function downloadPDF(Request $request)
    {
        ...
        $categories = DB::table('jenis_belanjas')
            ...
            ->where('jenis_belanjas.id_jenis_belanja', '9cdfcfed-6061-485b-8e20-662a776d0d06')
            ->where('jenis_kategori_rkbus.id_jenis_kategori_rkbu', '9cdfd329-6524-4d3f-aca5-93eade93046d')
        ...
        // $categories = JenisBelanja::where('id_jenis_belanja', '9cdfcfed-6061-485b-8e20-662a776d0d06') // Filter berdasarkan id_jenis_belanja
        //     ->whereHas('jenis_kategori_rkbus.obyek_belanjas.kategori_rkbus.sub_kategori_rkbus.rkbus.users', function ($query) use ($id_pejabat, $tahunAnggaran) {
        //         // Kondisi untuk id_jenis_kategori_rkbu yang spesifik dan tahun_anggaran
        //         $query->where('users.id_pejabat', $id_pejabat)
        //             ->where('jenis_kategori_rkbus.id_jenis_kategori_rkbu', '9cdfd329-6524-4d3f-aca5-93eade93046d')
        //             ->where('id_status_validasi_rka', '9cfb1f87-238b-4ea2-98f0-4255e578b1d1')
        //             ->where('nama_tahun_anggaran', $tahunAnggaran);
        //     })
        //     ->with([
        //         'jenis_kategori_rkbus' => function ($query) use ($id_pejabat, $tahunAnggaran) {
        //             $query->whereHas('obyek_belanjas.kategori_rkbus.sub_kategori_rkbus.rkbus.users', function ($query) use ($id_pejabat, $tahunAnggaran) {
        //                 // Kondisi untuk id_jenis_kategori_rkbu dan tahun_anggaran
        //                 $query->where('users.id_pejabat', $id_pejabat) // id_ksp berasal dari tabel users melalui relasi user
        //                     ->where('jenis_kategori_rkbus.id_jenis_kategori_rkbu', '9cdfd329-6524-4d3f-aca5-93eade93046d')

        //                     ->where('nama_tahun_anggaran', $tahunAnggaran);
        //             });
        //         },
        //         'jenis_kategori_rkbus.obyek_belanjas' => function ($query) use ($id_pejabat, $tahunAnggaran) {
        //             $query->whereHas('kategori_rkbus.sub_kategori_rkbus.rkbus.users', function ($query) use ($id_pejabat, $tahunAnggaran) {
        ...
        //             });
        //         },
        //         'jenis_kategori_rkbus.obyek_belanjas.kategori_rkbus' => function ($query) use ($id_pejabat, $tahunAnggaran) {
        //             $query->whereHas('sub_kategori_rkbus.rkbus.users', function ($query) use ($id_pejabat, $tahunAnggaran) {
        //                 // Tampilkan hanya data dengan id_user yang sesuai
        //                 $query->where('users.id_pejabat', $id_pejabat) // id_ksp berasal dari tabel users melalui relasi user
        //                     ->where('id_status_validasi_rka', '9cfb1f87-238b-4ea2-98f0-4255e578b1d1')
        //                     ->where('nama_tahun_anggaran', $tahunAnggaran);
        //             });
        //         },
        //         'jenis_kategori_rkbus.obyek_belanjas.kategori_rkbus.sub_kategori_rkbus' => function ($query) use ($id_pejabat, $tahunAnggaran) {
        //             $query->whereHas('rkbus.users', function ($query) use ($id_pejabat, $tahunAnggaran) {
        ...
        $htmlBody = view('frontend.prints.print_validasi_pegawai_pns_rka', compact('rkbus', 'tanggal_perencanaans', 'jabatan_kabag', 'nama_rumah_sakit', 'nama_pejabat', 'nip_pejabat_kabag', 'nip_direktur', 'nama_direktur', 'pdf', 'sub_kategori_rkbus', 'status_validasi_rka', 'total_anggaran', 'categories', 'namaProgram', 'namaKegiatan', 'namaSubKegiatan', 'kodeProgram', 'kodeKegiatan', 'kodeSubKegiatan'))->render();
