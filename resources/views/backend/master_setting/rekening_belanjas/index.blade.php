...
@section('container')
                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('rekening_belanjas.create')}}" class="btn btn-outline-secondary btn-primary">
        ...
        @else
                                <tr>
                                    @php $no = 1; @endphp
                                    @foreach ($rekening_belanjas as $ksp)
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $ksp->id_kode_rekening_belanja }}</td>
                                    <td>{{ $ksp->kode_rekening_belanja }}. {{ $ksp->nama_rekening_belanja }}</td>
                                    <td>{{ $ksp->aktivitas->kode_aktivitas }}. {{ $ksp->aktivitas->nama_aktivitas }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('rekening_belanjas.edit', $ksp->id_kode_rekening_belanja) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('rekening_belanjas.destroy', $ksp->id_kode_rekening_belanja) }}" method="POST" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
