        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                <ul class="dropdown-menu">
                    <li>
                        <a href="' . route('barang_assets.edit', $row->id_asset) . '" class="dropdown-item">
                            <i class="mdi mdi-pencil"></i> Edit
                        </a>
                    </li>
                    <li>
                        <form action="' . route('barang_assets.destroy', $row->id_asset) . '" method="POST" class="delete-form_db">
                    ...
                    <li>
                        <a href="' . route('barang_assets.PdfControllerSptjmAsset', $row->id_asset) . '" class="dropdown-item">
                            <i class="mdi mdi-printer"></i> SPTJM
                        </a>
                    </li>';

                if (!is_null($row->upload_sptjm)) {
                    $html .= '
                    <li>
                        <a href="' . route('barang_assets.download', $row->id_asset) . '" class="dropdown-item">
                            <i class="mdi mdi-download"></i> Download SPTJM
                        </a>
                    </li>';
                }
            ...
            ->addColumn('detail', function ($row) {
                ...
                <div style="padding: 5px; border-radius: 5px; width: 20ch;">
                    ...
                    <div style="font-size: 12px; padding: 2px 1px; border-radius: 3px; display: inline-block; margin-top: 0px;">Penempatan: ' . ($row->penempatan?->lokasi_barang ?? '-') . '</div></br>
            ...
            ->addColumn('actions', function ($row) {
                $buttons = '<div class="demo-inline-spacing">
                                <div class="d-flex justify-content-start align-items-center gap-1">';

                // Tombol Detail
                $buttons .= '<a href="' . route('barang_assets.details', ['id' => $row->id_asset]) . '" 
                ...
                // Tombol Generate QRCode
                $buttons .= '<a href="' . route('barang_assets.generate_qrcode', ['id_asset' => $row->id_asset]) . '" 
                                class="btn btn-icon btn-outline-primary waves-effect" data-bs-toggle="tooltip" title="Generate QrCode"
                                style="display: inline-block;">
                                <span class="tf-icons mdi mdi-qrcode-plus"></span>
                            </a>';

                // Tombol Reklas (jika kondisi_asset = "Rusak Berat")
                if ($row->kondisi_asset == "Rusak Berat") {
                    $buttons .= '<a href="' . route('barang_assets.reklas_update', ['id_asset' => $row->id_asset]) . '" 
                                    class="btn btn-icon btn-outline-danger waves-effect" data-bs-toggle="tooltip" title="Reklas Asset"
                                    style="display: inline-block;">
                                    <span class="tf-icons mdi mdi-repeat"></span>
                                </a>';
                }
            ...
            ->filterColumn('detail', function ($query, $keyword) {
                $query->where(function ($q) use ($keyword) {
                    $q->where('nama_asset', 'like', "%{$keyword}%")
                        ...
                        ->orWhere('pengguna_asset', 'like', "%{$keyword}%")
                        ->orWhere('id_penempatan', 'like', "%{$keyword}%")
    ...
    public function query(Asset $model): QueryBuilder
    {
        $query = $model->newQuery()
            ...
            ->where(function ($query) {
                $query->where('assets.status_reklas_arb', '!=', 'Reklas')
                    ->orWhereNull('assets.status_reklas_arb');
            });
        return $query->select([
            'assets.*',
