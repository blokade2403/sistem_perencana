...
class RkbuExportAdmin implements FromCollection, WithHeadings, WithEvents
{
    /**
    ...
    {
        // Pastikan session 'tahun_anggaran' tersedia
        if (!session()->has('tahun_anggaran')) {
            return collect([]); // Kembalikan koleksi kosong jika session tidak ditemukan
        }

        // Ambil tahun anggaran dari session
        $tahunAnggaran = session('tahun_anggaran');

        // Mengambil data RKBU beserta relasi ke tabel sub_kategori_rkbu, status_validasi, status_validasi_rka, dan rekening_belanja
        return Rkbu::with(['subKategoriRkbu', 'sub_kategori_rekenings', 'sub_kegiatans', 'status_validasi_rka', 'status_validasi', 'rekening_belanjas', 'user'])
            ...
            ->map(function ($rkbu) {
                return [
                    'id_sub_kategori_rekening'          => $rkbu->sub_kategori_rekenings->nama_sub_kategori_rekening ?? null,
                    'nama_sub_kategori_rkbu'            => $rkbu->subKategoriRkbu->nama_sub_kategori_rkbu ?? null,
                    'id_sub_kegiatan'                   => $rkbu->id_sub_kegiatan,
                    'nama_status_validasi'              => $rkbu->status_validasi->nama_validasi ?? null,
                    'nama_status_validasi_rka'          => $rkbu->status_validasi_rka->nama_validasi_rka ?? null,
                    'nama_rekening_belanja'             => $rkbu->rekening_belanjas->nama_rekening_belanja ?? null,
                    'id_user'                           => $rkbu->user->nama_lengkap ?? null,
