                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('sub_kategori_rekenings.create')}}" class="btn btn-outline-secondary btn-primary">
                        ...
                        <table id="example" class="table table-bordered" style="min-width: 845px">
                            <thead>
                                <tr>
                                    ...
                                    <th>Kode Sub Kategori Rekening</th>
                                    <th>Sub Kategori Rekening</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if($sub_kategori_rekenings->isEmpty())
        ...
        @else
                                <tr>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($sub_kategori_rekenings as $key)
                                    <td>{{ $no++}}</td>
                                    <td>{{ $key->kategori_rekening->kode_kategori_rekening }}. {{ $key->kategori_rekening->nama_kategori_rekening }} </td>
                                    <td>{{ $key->kode_sub_kategori_rekening }}</td>
                                    <td>{{ $key->nama_sub_kategori_rekening }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('sub_kategori_rekenings.edit', $key->id_sub_kategori_rekening) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('sub_kategori_rekenings.destroy', $key->id_sub_kategori_rekening) }}" method="POST" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
