                                    ...
                                    <div class="d-flex align-items-md-end align-items-sm-start align-items-center justify-content-md-between justify-content-start mx-4 flex-md-row flex-column gap-4">
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <a href="{{route('tanggal_perencanaans.create')}}" class="btn rounded-pill btn-label-info waves-effect">
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <table id="example" class="table table-bordered" style="min-width: 845px">
                            ...
                            <tbody>
                                @if($tanggal_perencanaans->isEmpty())
                                ...
                                <tr>
                                    @php $no = 1; @endphp
                                    @foreach ($tanggal_perencanaans as $ksp)
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $ksp->no_dpa }}</td>
                                    <td>{{ formatTanggal($ksp->tanggal) }}</td>
                                    <td>{{ $ksp->TahunAnggaran->nama_tahun_anggaran }}</td>
                                    <td>{{ $ksp->fase->nama_fase }}</td>
                                    <td>{{ $ksp->kota }}</td>
                                    <td>{{ $ksp->status }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('tanggal_perencanaans.edit', $ksp->id_tanggal_perencanaan) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('tanggal_perencanaans.destroy', $ksp->id_tanggal_perencanaan) }}" method="POST" class="delete-form">
                                                        @csrf
                                                        @method('DELETE')
