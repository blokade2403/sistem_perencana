<div class="pb-3 rounded-top">
    ...
    <div class="table-responsive">
        <table class="table m-0">
            ...
            <tbody>
                <tr>
                    <td class="text-wrap" style="width:150px">{{ $rkbuBarangJasa->nama_barang }}</td>
                    <td class="text-wrap" style="width: 200px;">{{ $rkbuBarangJasa->spek }}</td>
                    <td>
                        <span class="fa-sm">Stok Tahun {{$angka_kurang_2}} : {{$rkbuBarangJasa->stok}}</span><br />
                                        <span class="fa-sm">Rata2 Pemakaian {{$angka_kurang_1}} : {{$rkbuBarangJasa->rata_rata_pemakaian}}</span><br />
                                        <span class="fa-sm">Kebutuhan : {{$rkbuBarangJasa->kebutuhan_per_bulan}}</span><br />
                                        <span class="fa-sm">Pengadaan Tahun {{$angka_kurang_1}} : {{$rkbuBarangJasa->pengadaan_sebelumnya}}</span><br />
                                        <span class="fa-sm">Proyeksi Sisa Stok {{$angka_tahun}} : {{$rkbuBarangJasa->proyeksi_sisa_stok}}</span><br />
                                        <span class="fa-sm">Kebutuhan Tahun {{$angka_tahun}} : {{$rkbuBarangJasa->kebutuhan_tahun_x1}}</span><br />
                                        <span class="fa-sm">Buffer : {{$rkbuBarangJasa->buffer}}</span><br />
                                        <span class="fa-sm">Kebutuhan + Buffer : {{$rkbuBarangJasa->kebutuhan_plus_buffer}}</span><br />
                    </td>
                    <td>{{ number_format($rkbuBarangJasa->harga_satuan, 0, ',', '.') }}</td>
                    <td>{{$rkbuBarangJasa->rencana_pengadaan_tahun_x1}} {{$rkbuBarangJasa->satuan_1}}</td>
                </tr>
                <tr>
