...
class PdfControllerValidasiBarjasApbdAdmin extends Controller
{
    public function downloadPDF(Request $request)
    {
        ...
        $id_kde_rekeening = ['9cf6025b-eb56-4d10-9e00-17e74056b7f5', '9cf60308-4f19-4180-a2f1-87f7ed723add', '9cf60336-8954-46af-8d7d-dd32dd46b8a1'];
        ...
        $total_anggaran = Rkbu::join('users', 'rkbus.id_user', '=', 'users.id_user')
            ...
            ->where('jenis_kategori_rkbus.id_jenis_kategori_rkbu', '9cdfd354-e43a-4461-9747-e15fb74038ac')
            ->where('sub_kategori_rkbus.id_sub_kategori_rkbu', '!=', '9d03d062-b7d6-490e-ad71-10d92c5628cb')
            ->whereIn('rekening_belanjas.id_kode_rekening_belanja', $id_kde_rekeening)
            ->where('id_status_validasi_rka', '9cfb1f87-238b-4ea2-98f0-4255e578b1d1')
            ->sum('total_anggaran');
        ...
        if ($rkbus->isNotEmpty()) {
            ...
            $namaAktivitas  = optional($firstRkbu->rekening_belanjas->first()->aktivitas)->nama_aktivitas ?? 'Nama Aktivitas Tidak Ada';
            $sumberDana     = optional($firstRkbu->rekening_belanjas->first()->aktivitas->sub_kegiatan->sumber_dana)->nama_sumber_dana ?? 'Nama Aktivitas Tidak Ada';
        } else {
        ...
        $htmlBody = view('frontend.Ringkasan_Apbd.barang_jasa.print_validasi_barjas_apbd_admin', compact('rkbus', 'total_anggaran_terbilang', 'sumberDana', 'jabatan_kabag', 'nama_rumah_sakit', 'nama_pejabat', 'nip_pejabat_kabag', 'nip_direktur', 'nama_direktur', 'pdf', 'sub_kategori_rkbus', 'status_validasi_rka', 'total_anggaran', 'categories', 'namaProgram', 'namaAktivitas', 'namaKegiatan', 'namaSubKegiatan', 'kodeProgram', 'kodeKegiatan', 'kodeSubKegiatan'))->render();
