...
@section('container')
                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('aktivitass.create')}}" class="btn btn-outline-secondary btn-primary">
        ...
        @else
                                <tr>
                                    @php $no = 1; @endphp
                                    @foreach ($aktivitass as $ksp)
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $ksp->kode_aktivitas }} . {{ $ksp->nama_aktivitas }}</td>
                                    <td>{{ $ksp->program->kode_program }}. {{ $ksp->program->nama_program }}</td>
                                    <td>{{ $ksp->sub_kegiatan->kode_sub_kegiatan }}. {{ $ksp->sub_kegiatan->nama_sub_kegiatan }}</td>
                                    <td>{{ $ksp->sub_kegiatan->kegiatan->kode_kegiatan }}. {{ $ksp->sub_kegiatan->kegiatan->nama_kegiatan }}</td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('aktivitass.edit', $ksp->id_aktivitas) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                            <form action="{{ route('aktivitass.destroy', $ksp->id_aktivitas) }}" method="POST" class="delete-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="dropdown-item d-flex align-items-center">
                                                    <i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Hapus
                                                </button>
                                            </form>
