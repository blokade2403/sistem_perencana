...
class PdfControllerCekUsulanPersediaan extends Controller
{
    public function downloadPDF(Request $request, $id_usulan_barang)
    {
        ...
        // dd($sum_total_anggaran_usulan_barang);
        ...
        if ($invoice) {
            $keranjang = UsulanBarangDetail::where('usulan_barang_details.id_usulan_barang', $invoice->id_usulan_barang)
                ...
                ->select('usulan_barang_details.*', 'rkbus.nama_barang', 'rkbus.spek', 'rkbus.sisa_vol_rkbu', 'rkbus.sisa_anggaran_rkbu') // Select kolom yang diinginkan dari rkbus
                ->get();
        } else {
        ...
        $htmlBody = view('frontend.prints.print_cek_usulan_barang_persediaan', $invoice, $data, compact('rkbus', 'sub_kategori_rkbus', 'status_validasi_rka',  'namaProgram', 'namaKegiatan', 'namaSubKegiatan', 'kodeProgram', 'kodeKegiatan', 'kodeSubKegiatan'))->render();

        // Tuliskan konten body ke PDF
        $pdf->writeHTML($htmlBody, true, false, true, false, '');

        checkPageBreak($pdf, 65); // Memastikan ruang yang cukup untuk tanda tangan
        $htmlSignatureTable = '
            <table rules="none">
                <tr bgcolor="#ffffff">
        ...
        checkPageBreak($pdf, 20); // Memastikan ruang yang cukup untuk tanda tangan
        $htmlCatatan = '
            <table rules="none">
                <tr bgcolor="#ffffff">
                ...
                <tr bgcolor="#ffffff">
                    <th width="50%" align="Left">' . (!empty($no_inv->status_validasi_perencana) ? $invo->keterangan_perencana : '') . '</th>
                </tr>
                <tr bgcolor="#ffffff">
                    <th width="50%" align="Left">' . (!empty($no_inv->status_validasi_kabag) ? $invo->keterangan_kabag : '') . '</th>
                </tr>
                <tr bgcolor="#ffffff">
                    <th width="50%" align="Left">' . (!empty($no_inv->status_validasi_direktur) ? $invo->keterangan_direktur : '') . '</th>
                </tr>
            </table>';

        $pdf->writeHTML($htmlCatatan, true, false, true, false, '');
