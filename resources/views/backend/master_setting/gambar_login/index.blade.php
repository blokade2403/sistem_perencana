                                        ...
                                        <div class="key-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('gambar_logins.create')}}" class="btn btn-outline-secondary btn-primary">
                            ...
                            <table id="example" class="table table-striped" style="width:100%">
                                ...
                                <tbody>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach($gambarLogin as $key)
                                        <tr>
                                            <td>{{ $no++ }}</td>
                                            <td>{{ $key->tahun_anggaran }}</td>
                                            <td>
                                                @if($key->gambar_login)
                                                    <img src="{{ asset('storage/gambar_login/' . basename($key->gambar_login)) }}" 
                                                         alt="Gambar 1" 
                                            ...
                                            <td>{{ $key->status_gambar }}</td>
                                            <td>
                                                <div class="demo-inline-spacing">
                                                    <div class="d-flex justify-content-start align-items-center gap-2">
                                                        <a href="{{ route('gambar_logins.edit', $key->id_gambar_login) }}" 
                                                        ...
                                                        <form action="{{ route('gambar_logins.destroy', $key->id_gambar_login) }}" method="POST" class="delete-form">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" 
