                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('pejabats.create')}}" class="btn btn-outline-secondary btn-primary">
                        ...
                        <table id="example" class="table table-bordered" style="min-width: 845px">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $pejabat->id_pejabat }}</td>
                                    <td>{{ $pejabat->nama_pejabat }}</td>
                                    <td>{{ $pejabat->nip_pejabat }}</td>
                                    <td>{{ $pejabat->jabatan->nama_jabatan }}</td>
                                    <td>
                                        @if ($pejabat->status == 'aktif')
                                        <span class="badge bg-label-success">Aktif</span>
                                    @else
                                        <span class="badge bg-label-warning">Non Aktif</span>
                                    @endif
                                    </td>
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('pejabats.edit', $pejabat->id_pejabat) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <li>
                                                        <form action="{{ route('pejabats.destroy', $pejabat->id_pejabat) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
