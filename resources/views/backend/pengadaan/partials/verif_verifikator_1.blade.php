<form class="needs-validation" action="{{ route('master_spj.updateVerifVerifikator', $master_spj->id_master_spj) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="card mb-4">
        @if ($master_spj->status_verifikasi_verifikator == 'Selesai')
            <h5 class="card-header bg-label-success mb-4">Verifikasi Verifikator</h5>
        @else
            <h5 class="card-header bg-label-info mb-4">Verifikasi Verifikator</h5>
        @endif
        <div class="card-body">
                        ...
                        <div id="creditCardForm" class="row g-4">
                            <div class="col-12">
                                <span class="switch-label bg-label-danger rounded-1">
                                    <span class="fa-xs">Tanggal Verifikasi: {{ $master_spj->tgl_verif_verifikator ?? 'belum di verifikasi'}}</span>
                                </span>
                            </div>
                            <div class="col-12">
                                <div class="form-floating form-floating-outline">
                                    <select name="status_verifikasi_verifikator" id="satuan_1" class="select2 form-select form-select-lg" data-allow-clear="true">
                                        <option value="{{ $master_spj->status_verifikasi_verifikator }}">{{ $master_spj->status_verifikasi_verifikator }}</option>
                            ...
                            <div class="col-12">
                                <div class="form-floating form-floating-outline">
                                    <textarea class="form-control h-px-75" name="ket_verif_verifikator" aria-label="With textarea">{{$master_spj->ket_verif_verifikator}}</textarea>
