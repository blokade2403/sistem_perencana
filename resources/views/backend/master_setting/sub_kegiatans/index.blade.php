                                        ...
                                        <div class="user-profile-info">
                                            ...
                                            <ul class="list-inline mb-0 d-flex align-items-center flex-wrap justify-content-sm-start justify-content-center gap-2">
                                                <li class="list-inline-item">
                                                    <i class="mdi mdi-map-marker-outline me-1 mdi-20px"></i><span class="fw-semibold">{{session('nama_unit')}}</span>
                                                </li>
                                                <li class="list-inline-item">
                                            ...
                                            <div class="btn-toolbar demo-inline-spacing" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group" role="group" aria-label="First group">
                                                    <a href="{{route('sub_kegiatans.create')}}" class="btn btn-outline-secondary btn-primary">
        ...
        @else
                                <tr>
                                    @php
                                        $no = 1;
                                    @endphp
                                    @foreach ($sub_kegiatans as $key)
                                    <td>{{ $no++}}</td>
                                    <td>{{ $key->kegiatan->program->kode_program }}. {{ $key->kegiatan->program->nama_program }} </td>
                                    <td>{{ $key->kegiatan->kode_kegiatan }}. {{ $key->kegiatan->nama_kegiatan }}</td>
                                    <td>{{ $key->kode_sub_kegiatan }}. {{ $key->nama_sub_kegiatan }}</td>
                                    <td>{{ $key->tujuan_sub_kegiatan }}</td>
                                    <td>{{ $key->indikator_sub_kegiatan }}</td>
                                        ...
                                        <td>
                                            <div class="btn-group" id="dropdown-icon-demo">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('sub_kegiatans.edit', $key->id_sub_kegiatan) }}" class="dropdown-item d-flex align-items-center"><i class="mdi mdi-chevron-right scaleX-n1-rtl"></i>Edit</a>
                                                    </li>
                                                    <form action="{{ route('sub_kegiatans.destroy', $key->id_sub_kegiatan) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
